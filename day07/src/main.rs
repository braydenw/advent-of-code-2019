...
fn process_sequence(amplifiers: &mut Vec<IntcodeVM>, phase_settings: [i32; 5]) -> i32 {
    let mut output = 0;

    for i in 0..5 {
        // First input
        amplifiers[i].push_input(phase_settings[i]);

        // Second input
        amplifiers[i].push_input(output);

        // Run the program until halting or it needs input.
        while let IntcodeState::Running = amplifiers[i].step() {
            // ...
        }

        // Store the last output.
        output = *amplifiers[i].outputs().last().unwrap();
    }
...
fn process_sequence_loop(amplifiers: &mut Vec<IntcodeVM>, phase_settings: [i32; 5]) -> i32 {
...
fn permutations(set: [i32; 5]) -> Vec<[i32; 5]> {
    let mut perms: Vec<[i32; 5]> = Vec::new();

    let mut ints: [i32; 5] = set;
    let mut swaps: [i32; 5] = [0; 5];
    let mut current = 0;
    'outer: loop {
        if current > 0 {
            'inner: loop {
                ...
                if swaps[current] < current as i32 {
                    break 'inner;
                }
